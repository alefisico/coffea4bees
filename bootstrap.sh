...
if [ "\$1" == "" ]; then
  #export COFFEA_IMAGE=registry.hub.docker.com/coffeateam/coffea-dask:latest
  export COFFEA_IMAGE=gitlab-registry.cern.ch/cms-cmu/coffea4bees:latest
else
...
SINGULARITY_SHELL=\$(which bash) singularity exec -B \${PWD}:/srv -B /cvmfs -B /uscmst1b_scratch --pwd /srv \\
  /cvmfs/unpacked.cern.ch/\${COFFEA_IMAGE} \\
