...
from coffea4bees.stats_analysis.make_variable_binning import rebin_histogram
...
_custom_colors_index = []
_custom_colors = []
def get_color(input_color):

    if type(eval(input_color)) == tuple:
        _custom_colors_index.append(ROOT.TColor.GetFreeColorIndex())
        color_RGB = eval(input_color)
        _custom_colors.append(ROOT.TColor(_custom_colors_index[-1], color_RGB[0], color_RGB[1], color_RGB[2], "", 1 ))
        return _custom_colors_index[-1]

    return eval(input_color)
...
def make_ratio(rPad, numer, denom, rMin, rMax, rTitle, rColor, lColor, ratioTObjects=[],ratioErrors=True, doSignificance=False, plotParameters=None, numerSample=None, denomSample=None, drawOptions=None):
    ...
    for f in sampleDictionary:
        ...
        for p in sampleDictionary[f]:
            ...
            if "color" in thisSample:
                thisHist.SetLineColor(get_color(thisSample["color"]))
                thisHist.SetMarkerColor(get_color(thisSample["color"]))
                ...
                if "stack" in thisSample and "TH2" not in str(type(thisHist)):
                    thisHist.SetFillColor(get_color(thisSample["color"]))
    ...
    if ratio or th2Ratio:
        if stackRatio:
            ...
            if "signal" in stackRatio:
                dN_N(stacked.GetStack().Last(), True,  stackRatio.replace("signal",""), rPad, rMin, rMax)
            if "bkgd"   in stackRatio:
                dN_N(stacked.GetStack().Last(), False, stackRatio.replace("bkgd"  ,""), rPad, rMin, rMax)
            ...
            for p in sampleDictionary[f]:
                ...
                if "stack" not in thisSample and "ratio" in thisSample:
                    ...
                    if "signal" in histRatio:
                        dN_N(thisHist, True,  histRatio.replace("signal",""), rPad, rMin, rMax)
                    if "bkgd"   in histRatio:
                        dN_N(thisHist, False, histRatio.replace("bkgd"  ,""), rPad, rMin, rMax)
            ...
            for denom, denomSample in ratioDictionary["denom"][ratioSet]:
                ...
                for numer, numerSample in ratioDictionary["numer"][ratioSet]:
                    ...
                    if not th2Ratio:
                        ...
                        make_ratio(rPad, numer, denom, rMin, rMax, rTitle, rColor, lColor, ratioTObjects, ratioErrors, doSignificance,plotParameters, numerSample, denomSample)
                    else:
                        hPad.cd()
                        try:
