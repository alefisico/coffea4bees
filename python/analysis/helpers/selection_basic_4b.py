...
def apply_event_selection_4b( event, isMC, corrections_metadata ):
    ...
    event['passHLT'] = np.full(len(event), True) if isMC else mask_event_decision( event, decision="OR", branch="HLT", list_to_mask=event.metadata['trigger']  )

    event['passNoiseFilter'] = mask_event_decision( event, decision="AND", branch="Flag",
                                        list_to_mask=corrections_metadata['NoiseFilter'],
                                        list_to_skip=['BadPFMuonDzFilter', 'hfNoisyHitsFilter']  )

    return event

def apply_object_selection_4b( event, year, isMC, dataset, corrections_metadata ):
    ...
    event['Jet', 'calibration'] = event.Jet.pt / ( 1 if 'pt_raw' in event.Jet.fields else event.Jet.pt_raw )    # AGE: I include the mix condition, I think it is wrong, to check later
    event['Jet', 'lepton_cleaned'] = drClean( event.Jet,event.selLepton )[1]  ### 0 is the collection of jets, 1 is the flag
