...
class analysis(processor.ProcessorABC):
    def __init__(
        ...
        *,
        ...
        self.classifier_SvB_MA = None
        if SvB or SvB_MA:
            from analysis.helpers.networks import HCREnsemble

            if SvB:
                self.classifier_SvB = HCREnsemble(SvB)
            if SvB_MA:
                self.classifier_SvB_MA = HCREnsemble(SvB_MA)
    ...
    def process(self, event):
        ...
        if self.apply_FvT:
            event["FvT"] = (
                NanoEventsFactory.from_root(
                    f'{path}{"FvT_3bDvTMix4bDvT_v0_newSB.root" if "mix" in dataset else "FvT.root"}',
                    entry_start=estart,
                    entry_stop=estop,
        ...
        if isMC:
            ...
            logging.debug(
                f"event['weight'] = event.genWeight * (lumi * xs * kFactor / genEventSumw) = {event.genWeight[0]} * ({lumi} * {xs} * {kFactor} / {genEventSumw}) = {event.weight[0]}\n"
            )
            ...
            for var in ["nominal", "up", "down"]:
                event[f"PU_weight_{var}"] = puWeight.evaluate(
                    event.Pileup.nTrueInt.to_numpy(), var
                )
        ...
        self._cutFlow.fill(
            "passNoiseFilter",
            event[event.lumimask & event.passNoiseFilter],
            allTag=True,
        )
        self._cutFlow.fill(
            "passHLT",
            event[event.lumimask & event.passNoiseFilter & event.passHLT],
            allTag=True,
        )

        # Apply object selection (function does not remove events, adds content to objects)
        event = apply_object_selection_4b(
            event, year, isMC, dataset, self.corrections_metadata[year]
        )
        selections = []
        selections.append(
            event.lumimask & event.passNoiseFilter & event.passHLT & event.passJetMult
        )
        ...
        if isMC and self.apply_btagSF:
            btagSF = correctionlib.CorrectionSet.from_file(
                self.corrections_metadata[year]["btagSF"]
            )["deepJet_shape"]
            selev["weight"] = apply_btag_sf(
                selev,
                selev.selJet,
        ...
        selev["notCanJet"] = notCanJet
        selev["nNotCanJet"] = ak.num(selev.notCanJet)
        ...
        if self.JCM:
            ...
            if self.apply_FvT:
                ...
                weight[selev.threeTag] = (
                    selev.weight[selev.threeTag]
                    * pseudoTagWeight[selev.threeTag]
                    * selev.FvT.FvT[selev.threeTag]
                )
                selev["weight"] = weight
            else:
        ...
        if self.run_topreco:
            ...
            selev["top_cand", "xWt"] = np.sqrt(
                selev.top_cand.xW**2 + selev.top_cand.xt**2
            )
            selev["top_cand", "mbW"] = (bReg_p + selev.top_cand.W.pW).mass
        ...
        fill += hist.add(
            "hT_selected",
            (100, 0, 1000, ("hT_selected", "H_{T} (selected jets) [GeV}")),
        )
        ...
        fill += Jet.plot(("othJets", "Other Jets"), "notCanJet", skip=["deepjet_c"])
    ...
    def compute_SvB(self, event):
        ...
        o[:, 0, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(ak.pad_none(event.notCanJet.pt, target=8, clip=True)),
                0,
            )
        )
        o[:, 1, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(ak.pad_none(event.notCanJet.eta, target=8, clip=True)),
                0,
            )
        )
        o[:, 2, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(ak.pad_none(event.notCanJet.phi, target=8, clip=True)),
                0,
            )
        )
        o[:, 3, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(ak.pad_none(event.notCanJet.mass, target=8, clip=True)),
                0,
            )
        )
        o[:, 4, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(
                    ak.pad_none(event.notCanJet.isSelJet, target=8, clip=True)
                ),
                -1,
            )
        )
