...
def setSvBVars(SvBName, event):
    ...
    event[SvBName, "passMinPs"] = (
        (getattr(event, SvBName).pzz > 0.01)
        | (getattr(event, SvBName).pzh > 0.01)
        | (getattr(event, SvBName).phh > 0.01)
    )
    event[SvBName, "zz"] = (
        getattr(event, SvBName).pzz > getattr(event, SvBName).pzh
    ) & (getattr(event, SvBName).pzz > getattr(event, SvBName).phh)
    event[SvBName, "zh"] = (
        getattr(event, SvBName).pzh > getattr(event, SvBName).pzz
    ) & (getattr(event, SvBName).pzh > getattr(event, SvBName).phh)
    event[SvBName, "hh"] = (
        getattr(event, SvBName).phh >= getattr(event, SvBName).pzz
    ) & (getattr(event, SvBName).phh >= getattr(event, SvBName).pzh)
    event[SvBName, "largest"] = largest_name[
        getattr(event, SvBName).passMinPs
        * (
            1 * getattr(event, SvBName).zz
            + 2 * getattr(event, SvBName).zh
            + 3 * getattr(event, SvBName).hh
        )
    ]
    ...
    this_ps_zz[getattr(event, SvBName).zz] = getattr(event, SvBName).pzz[
        getattr(event, SvBName).zz
    ]
    event[SvBName, "ps_zz"] = this_ps_zz

    this_ps_zh = np.full(len(event), -1, dtype=float)
    this_ps_zh[getattr(event, SvBName).zh] = getattr(event, SvBName).pzh[
        getattr(event, SvBName).zh
    ]
    event[SvBName, "ps_zh"] = this_ps_zh

    this_ps_hh = np.full(len(event), -1, dtype=float)
    this_ps_hh[getattr(event, SvBName).hh] = getattr(event, SvBName).phh[
        getattr(event, SvBName).hh
    ]
...
class analysis(processor.ProcessorABC):
    ...
    def process(self, event):
        ...
        if isMC:
            ...
            if not isTTForMixed:
                puWeight = list(
                    correctionlib.CorrectionSet.from_file(
                        self.corrections_metadata[year]["PU"]
                    ).values()
                )[0]
                weights.add(
                    "PU",
            ...
            if "LHEPdfWeight" in event.fields:
                ...
                if "306000 - 306102" in event.LHEPdfWeight.__doc__:
                    ...
                    arg = event.LHEPdfWeight[:, 1:-2] - np.ones(
                        (len(weights.weight()), 100)
                    )
        ...
        #
        if (
            isMixedData or isDataForMixed or isTTForMixed or not isMC
        ):  #### AGE: data corrections are not applied. Should be changed
            jets = event.Jet

        else:
            juncWS = [
                self.corrections_metadata[year]["JERC"][0].replace("STEP", istep)
        ...
        if self.run_systematics:
            ...
            shifts.extend(
                [({"Jet": jets.JER.up}, "JER_Up"), ({"Jet": jets.JER.down}, "JER_Down")]
            )
    ...
    def process_shift(self, event, shift_name, weights):
        ...
        if self.run_SvB:
            if (self.classifier_SvB is None) | (self.classifier_SvB_MA is None):
                event["SvB"] = (
                    NanoEventsFactory.from_root(
                        f'{path}{"SvB_newSBDef.root" if "mix" in dataset else "SvB.root"}',
                        entry_start=estart,
                        entry_stop=estop,
                        schemaclass=FriendTreeSchema,
                    )
                ...
                event["SvB_MA"] = (
                    NanoEventsFactory.from_root(
                        f'{path}{"SvB_MA_newSBDef.root" if "mix" in dataset else "SvB_MA.root"}',
                        entry_start=estart,
                        entry_stop=estop,
                        schemaclass=FriendTreeSchema,
                    )
        ...
        selections.add(
            ...
            (
                ...
                if (isMC or isMixedData or isTTForMixed or isDataForMixed)
                else event.passHLT
            ),
        )
        if not shift_name:
        ...
        event = apply_object_selection_4b(
            event,
            year,
            isMC,
            dataset,
        ...
        if isMC and self.apply_btagSF:
            ...
            if (not shift_name) & self.run_systematics:
                btag_SF_weights = apply_btag_sf(
                    event.selJet,
                    correction_file=self.corrections_metadata[year]["btagSF"],
                    btag_uncertainties=self.corrections_metadata[year][
                        "btag_uncertainties"
                    ],
                )
                weights.add_multivariation(
                    ...
                    [
                        var.to_numpy()
                        for name, var in btag_SF_weights.items()
                        if "_up" in name
                    ],
                    [
                        var.to_numpy()
        ...
        #
        sorted_idx = ak.argsort(
            selev.Jet.btagDeepFlavB * selev.Jet.selected, axis=1, ascending=False
        )
        ...
        notCanJet["isSelJet"] = 1 * (
            (notCanJet.pt > 40) & (np.abs(notCanJet.eta) < 2.4)
        )  # should have been defined as notCanJet.pt>=40, too late to fix this now...
        ...
        if self.JCM:
            selev["Jet_untagged_loose"] = selev.Jet[
                selev.Jet.selected & ~selev.Jet.tagged_loose
            ]
            ...
            weight_noFvT[selev.threeTag] = (
                selev.weight[selev.threeTag] * selev.pseudoTagWeight[selev.threeTag]
            )
            ...
            if self.apply_FvT:
                ...
                if isDataForMixed:
                    ...
                    #
                    for _JCM_load, _FvT_name in zip(
                        event.metadata["JCM_loads"], event.metadata["FvT_names"]
                    ):
                        _weight = np.array(selev.weight.to_numpy(), dtype=float)
                        _weight[selev.threeTag] = (
                            selev.weight[selev.threeTag]
                            * getattr(selev, f"{_JCM_load}")[selev.threeTag]
                            * getattr(getattr(selev, _FvT_name), _FvT_name)[
                                selev.threeTag
                            ]
                        )
                    ...
                    weight = (
                        pseudoTagWeight[selev.threeTag] * selev.FvT.FvT[selev.threeTag]
                    )
        ...
        diJet["passDiJetMass"] = (minDiJetMass < diJet.mass) & (
            diJet.mass < maxDiJetMass
        )
        ...
        diJet["passMDR"] = (min_m4j_scale / m4j + min_dr_offset < diJet.dr) & (
        ...
        selev.selJet = selev.selJet[
            ak.argsort(selev.selJet.btagDeepFlavB, axis=1, ascending=False)
        ]
        ...
        # mW, mt = 80.4, 173.0
        selev["top_cand", "W"] = ak.zip(
            {
                "p": selev.top_cand.j + selev.top_cand.l,
                "j": selev.top_cand.j,
                "l": selev.top_cand.l,
            }
        )

        selev["top_cand", "W", "pW"] = selev.top_cand.W.p * (
            mW / selev.top_cand.W.p.mass
        )
        selev["top_cand", "mbW"] = (bReg_p + selev.top_cand.W.pW).mass
        selev["top_cand", "xt"] = (selev.top_cand.p.mass - mt) / (
            0.10 * selev.top_cand.p.mass
        )
        selev["top_cand", "xWt"] = np.sqrt(selev.top_cand.xW**2 + selev.top_cand.xt**2)
        selev["top_cand", "mbW"] = (bReg_p + selev.top_cand.W.pW).mass
        selev["top_cand", "xWbW"] = np.sqrt(
            selev.top_cand.xW**2 + selev.top_cand.xbW**2
        )
        ...
        #
        selev["top_cand", "rWbW"] = np.sqrt(
            (selev.top_cand.xbW - 0.25) ** 2 + (selev.top_cand.xW - 0.5) ** 2
        )
        ...
        if self.apply_FvT:
            quadJet["FvT_q_score"] = np.concatenate(
                (
                    np.reshape(np.array(selev.FvT.q_1234), (-1, 1)),
        ...
        if self.run_SvB:
            ...
            quadJet["SvB_q_score"] = np.concatenate(
                (
                    np.reshape(np.array(selev.SvB.q_1234), (-1, 1)),
            ...
            quadJet["SvB_MA_q_score"] = np.concatenate(
                (
                    np.reshape(np.array(selev.SvB_MA.q_1234), (-1, 1)),
                    np.reshape(np.array(selev.SvB_MA.q_1324), (-1, 1)),
                    np.reshape(np.array(selev.SvB_MA.q_1423), (-1, 1)),
                ),
                axis=1,
            )
        #
        ...
        selev["quadJet_min_dr"] = quadJet[
            np.array(range(len(quadJet))), arg_min_close_dr
        ]
        ...
        selev["region"] = (
            selev["quadJet_selected"].SR * 0b10 + selev["quadJet_selected"].SB * 0b01
        )
        if self.run_SvB:
        ...
        if not shift_name:
            ...
            if self.run_SvB:
                self._cutFlow.fill(
                    "SR", selev[(selev.passDiJetMass & selev["quadJet_selected"].SR)]
                )
                self._cutFlow.fill(
                    "SB", selev[(selev.passDiJetMass & selev["quadJet_selected"].SB)]
                )
        ...
        if not self.run_systematics:
            ...
            fill += hist.add(
                "nPVs", (101, -0.5, 100.5, ("PV.npvs", "Number of Primary Vertices"))
            )
            fill += hist.add(
                "nPVsGood",
                (101, -0.5, 100.5, ("PV.npvsGood", "Number of Good Primary Vertices")),
            )
            ...
            if self.apply_FvT:
                ...
                if "xbW" in selev.fields:  ### AGE: this should be temporary
                    ...
                    fill += hist.add(
                        "delta_xW_l", (100, -15, 15, ("delta_xW", "delta xW"))
                    )
            ...
            if isDataForMixed:
                for _FvT_name in event.metadata["FvT_names"]:
                    fill += SvBHists(
                        (f"SvB_{_FvT_name}", "SvB Classifier"),
                        "SvB",
                        weight=f"weight_{_FvT_name}",
                    )
                    fill += SvBHists(
                        (f"SvB_MA_{_FvT_name}", "SvB MA Classifier"),
                        "SvB_MA",
                        weight=f"weight_{_FvT_name}",
                    )
            ...
            fill += Jet.plot(
                ("canJets", "Higgs Candidate Jets"), "canJet", skip=["deepjet_c"]
            )
            fill += Jet.plot(
                ("othJets", "Other Jets"), "notCanJet_coffea", skip=["deepjet_c"]
            )
            ...
            #  Make quad jet hists
            #
            fill += LorentzVector.plot_pair(
                ("v4j", R"$HH_{4b}$"),
                "v4j",
                skip=["n", "dr", "dphi", "st"],
                bins={"mass": (120, 0, 1200)},
            )
            fill += QuadJetHists(
                ("quadJet_selected", "Selected Quad Jet"), "quadJet_selected"
            )
            fill += QuadJetHists(
                ("quadJet_min_dr", "Min dR Quad Jet"), "quadJet_min_dr"
            )
            ...
            if self.apply_FvT:
                ...
                fill += hist.add(
                    "quadJet_selected_FvT_score",
                    (
                        100,
                        0,
                        1,
                        (
                            "quadJet_selected.FvT_q_score",
                            "Selected Quad Jet Diboson FvT q score",
                        ),
                    ),
                )
                fill += hist.add(
                    "quadJet_min_FvT_score",
                    (
                        100,
                        0,
                        1,
                        (
                            "quadJet_min_dr.FvT_q_score",
                            "Min dR Quad Jet Diboson FvT q score",
                        ),
                    ),
                )
                if self.JCM:
            ...
            fill += Jet.plot(
                ("selJets_noJCM", "Selected Jets"),
                "selJet",
                weight="weight_noJCM_noFvT",
                skip=skip_all_but_n,
            )
            fill += Jet.plot(
                ("tagJets_noJCM", "Tag Jets"),
                "tagJet",
                weight="weight_noJCM_noFvT",
                skip=skip_all_but_n,
            )
            fill += Jet.plot(
                ("tagJets_loose_noJCM", "Loose Tag Jets"),
                "tagJet_loose",
                weight="weight_noJCM_noFvT",
                skip=skip_all_but_n,
            )
            ...
            for iJ in range(4):
                fill += Jet.plot(
                    (f"canJet{iJ}", f"Higgs Candidate Jets {iJ}"),
                    f"canJet{iJ}",
                    skip=["n", "deepjet_c"],
                )
            ...
            fill += Muon.plot(
                ("selMuons", "Selected Muons"), "selMuon", skip=skip_muons
            )
            ...
            if not isMixedData:
                ...
                fill += Elec.plot(
                    ("selElecs", "Selected Elecs"), "selElec", skip=skip_elecs
                )
        ...
        #
            ...
            fill_SvB = Fill(
                process=processName, year=year, variation=shift_name, weight="weight"
            )
            fill_SvB += SvBHists(("SvB", "SvB Classifier"), "SvB", skip=["ps", "ptt"])
            fill_SvB += SvBHists(
                ("SvB_MA", "SvB MA Classifier"), "SvB_MA", skip=["ps", "ptt"]
            )
            ...
            if "nominal" in shift_name:
                ...
                for ivar in list(weights.variations):
                    ...
                    selev[f"weight_{ivar}"] = weights.weight(modifier=ivar)[
                        selections.all(*allcuts)
                    ]
                    fill_SvB_ivar = Fill(
                        process=processName,
                        year=year,
                        variation=ivar,
                        weight=f"weight_{ivar}",
                    )
                    logging.debug(f"{ivar} {selev['weight']}")
                    fill_SvB_ivar += SvBHists(
                        ("SvB", "SvB Classifier"), "SvB", skip=["ps", "ptt"]
                    )
                    fill_SvB_ivar += SvBHists(
                        ("SvB_MA", "SvB MA Classifier"), "SvB_MA", skip=["ps", "ptt"]
                    )
                    ...
                    for ih in hist_SvB.output["hists"].keys():
                        hist_SvB.output["hists"][ih] = (
                            hist_SvB.output["hists"][ih]
                            + dict_hist_SvB[ivar].output["hists"][ih]
                        )
    ...
    def compute_SvB(self, event):
        ...
        o[:, 0, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(
                    ak.pad_none(event.notCanJet_coffea.pt, target=8, clip=True)
                ),
                0,
            )
        )
        o[:, 1, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(
                    ak.pad_none(event.notCanJet_coffea.eta, target=8, clip=True)
                ),
                0,
            )
        )
        o[:, 2, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(
                    ak.pad_none(event.notCanJet_coffea.phi, target=8, clip=True)
                ),
                0,
            )
        )
        o[:, 3, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(
                    ak.pad_none(event.notCanJet_coffea.mass, target=8, clip=True)
                ),
                0,
            )
        )
        o[:, 4, :] = torch.tensor(
            ak.fill_none(
                ak.to_regular(
                    ak.pad_none(event.notCanJet_coffea.isSelJet, target=8, clip=True)
                ),
                -1,
            )
        )
        ...
        for classifier in ["SvB", "SvB_MA"]:
            ...
            c_score, q_score = (
                F.softmax(c_logits, dim=-1).numpy(),
                F.softmax(q_logits, dim=-1).numpy(),
            )
            ...
            if classifier in event.fields:
                ...
                if np.any(error):
                    ...
                    logging.warning(
                        f"WARNING: Calculated {classifier} does not agree "
                        f"within tolerance for some events ({np.sum(error)}/{len(error)})",
                        delta[worst],
                    )
                    logging.warning("----------")
