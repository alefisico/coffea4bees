...
class analysis(processor.ProcessorABC):
    ...
    def process(self, event):
        ...
        if self.top_reconstruction_override:
            ...
            logging.info(
                f"top_reconstruction overridden to {self.top_reconstruction}\n"
            )
        else:
        ...
        if self.run_SvB:
            if (self.classifier_SvB is None) | (self.classifier_SvB_MA is None):
                ...
                SvB_file = (
                    f"{path}/SvB_ULHH.root"
                    if "mix" in self.dataset
                    else f'{fname.replace("picoAOD", "SvB_ULHH")}'
                )
                event["SvB"] = (
                    NanoEventsFactory.from_root(
                        SvB_file,
                ...
                SvB_MA_file = (
                    f"{path}/SvB_MA_ULHH.root"
                    if "mix" in self.dataset
                    else f'{fname.replace("picoAOD", "SvB_MA_ULHH")}'
                )
                event["SvB_MA"] = (
                    NanoEventsFactory.from_root(
                        SvB_MA_file,
        ...
        # Event selection
        #
        event = apply_event_selection_4b(
            event,
        ...
        if self.apply_boosted_veto:
            ...
            boosted_events = (
                boosted_file[self.dataset]["event"]
                if self.dataset in boosted_file.keys()
                else event.event
            )
        ...
        if self.run_systematics:
            for jesunc in self.corrections_metadata[self.year]["JES_uncertainties"]:
                shifts.extend(
                    [
                        ({"Jet": jets[f"JES_{jesunc}"].up}, f"CMS_scale_j_{jesunc}Up"),
                        (
                            {"Jet": jets[f"JES_{jesunc}"].down},
                            f"CMS_scale_j_{jesunc}Down",
                        ),
                    ]
                )
            ...
            shifts.extend(
                [
                    ({"Jet": jets.JER.up}, f"CMS_res_j_{self.year_label}Up"),
                    ({"Jet": jets.JER.down}, f"CMS_res_j_{self.year_label}Down"),
                ]
            )
        ...
        return processor.accumulate(
            self.process_shift(
                update_events(event, collections),
                name,
                weights,
                list_weight_names,
                target,
    ...
    def process_shift(self, event, shift_name, weights, list_weight_names, target):
        ...
        event = apply_object_selection_4b(
            ...
            override_selected_with_flavor_bit=self.config[
                "override_selected_with_flavor_bit"
            ],
        ...
        selections.add(
            ...
            (
                event.passHLT
                if self.config["cut_on_HLT_decision"]
        ...
        if not shift_name:
            ...
            processOutput["nEvent"][event.metadata["dataset"]] = {
                "nEvent": self.nEvent,
                "genWeights": (
                    np.sum(event.genWeight) if self.config["isMC"] else self.nEvent
                ),
            }
        ...
        if self.config["isMC"] and self.apply_btagSF:
            ...
            logging.debug(
                f"Btag weight {weights.partial_weight(include=['CMS_btag'])[:10]}\n"
            )
        ...
        if self.subtract_ttbar_with_weights:
            ...
            pass_ttbar_filter_selev = subtract_ttbar_with_SvB(
                selev, self.dataset, self.year
            )
            ...
            self._cutFlow.fill(
                "pass_ttbar_filter", event[selections.all(*allcuts)], allTag=True
            )
        ...
        if self.friend_top_reconstruction:  ## temporary until we create friend trees
            with open(self.friend_top_reconstruction, "r") as f:
                self.friend_top_reconstruction = Friend.from_json(
                    json.load(f)[f'top_reco{"_"+shift_name if shift_name else ""}']
                )
            top_cand = self.friend_top_reconstruction.arrays(target)[
                analysis_selections
            ]
        ...
        if not (self.config["isMC"] or "mixed" in self.dataset) and self.blind:
            ...
            blind_sel[analysis_selections] = ~(
                selev["quadJet_selected"].SR & selev.fourTag
            )
        ...
        selev["trigWeight"] = weights.partial_weight(
            include=["CMS_bbbb_resolved_ggf_triggerEffSF"]
        )[analysis_selections]
        ...
        if not shift_name:
            ...
            self._cutFlow.fill(
                "passDiJetMass_woTrig",
                selev[selev.passDiJetMass],
                wOverride=np.sum(
                    weights.partial_weight(
                        exclude=["CMS_bbbb_resolved_ggf_triggerEffSF"]
                    )[analysis_selections][selev.passDiJetMass]
                ),
            )
            if self.run_SvB:
                ...
                self._cutFlow.fill(
                    ...
                    wOverride=np.sum(
                        weights.partial_weight(
                            exclude=["CMS_bbbb_resolved_ggf_triggerEffSF"]
                        )[analysis_selections][selev.passSR]
                    ),
                )
                ...
                self._cutFlow.fill(
                    "SB_woTrig",
                    selev[(selev.passDiJetMass & selev["quadJet_selected"].SB)],
                    wOverride=np.sum(
                        weights.partial_weight(
                            exclude=["CMS_bbbb_resolved_ggf_triggerEffSF"]
                        )[analysis_selections][selev.passSB]
                    ),
                )
                ...
                self._cutFlow.fill(
                    ...
                    wOverride=np.sum(
                        weights.partial_weight(
                            exclude=["CMS_bbbb_resolved_ggf_triggerEffSF"]
                        )[analysis_selections][selev.passSvB]
                    ),
                )
                ...
                self._cutFlow.fill(
                    ...
                    wOverride=np.sum(
                        weights.partial_weight(
                            exclude=["CMS_bbbb_resolved_ggf_triggerEffSF"]
                        )[analysis_selections][selev.failSvB]
                    ),
                )
        ...
        if self.make_top_reconstruction is not None:
            ...
            friends["friends"] = friends["friends"] | dump_top_reconstruction(
                selev,
        ...
        if self.make_classifier_input is not None:
            ...
            friends["friends"] = friends["friends"] | dump_input_friend(
                selev,
        ...
        if self.make_friend_JCM_weight is not None:
            ...
            friends["friends"] = friends["friends"] | dump_JCM_weight(
                selev, self.make_classifier_input, "JCM_weight", analysis_selections
            )

        if self.make_friend_FvT_weight is not None:
            from ..helpers.dump_friendtrees import dump_FvT_weight

            friends["friends"] = friends["friends"] | dump_FvT_weight(
                selev, self.make_classifier_input, "FvT_weight", analysis_selections
            )
