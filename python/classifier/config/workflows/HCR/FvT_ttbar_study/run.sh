...
for i in {0..14}
do
    ./pyml.py template "{mixed: ${i}, offset: 0, user: ${LPCUSER}, tag: mixed}" $WFS/train.yml -setting Monitor "address: :${port}"
...
for i in {0..14}
do
    ./pyml.py template "{mixed: ${i}, tag: mixed, user: ${LPCUSER}}" $WFS/evaluate.yml -setting Monitor "address: :${port}"
done