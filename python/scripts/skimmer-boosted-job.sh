...
if [[ $(hostname) = *fnal* ]]; then
    ...
    sed -e "s#base_.*#base_path: \/builds\/${CI_PROJECT_PATH}\/output\/skimmer_boosted_job\/#" -e "s/\#max.*/maxchunks: 5/" -e "s/\#test.*/test_files: 1/" -e "s/2024_.*/tmp\//" -e "s/T3_US_FNALLPC/T3_CH_PSI/" skimmer/metadata/HH4b_boosted.yml > $OUTPUT_DIR/HH4b_boosted.yml
fi
